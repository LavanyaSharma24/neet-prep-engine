...
- **Tier 2 cross-check currently uses two same-family Flash models, not a genuine Flash/Pro cross-tier pair.** The original design intent for Tier 2 cross-checking was two architecturally-independent models (e.g. a Flash-class and a Pro-class model), on the reasoning that same-family models are more likely to share correlated blind spots than models from different tiers. Verified directly against the live Gemini API (2026-09-13, not assumed from documentation or config comments): the current API key has zero free-tier quota for every Pro-class model (`gemini-pro-latest`, `gemini-3.1-pro-preview`) — a hard billing gate (429, `limit: 0`), not a transient rate limit — while `gemini-flash-latest` and `gemini-flash-lite-latest` both work. Tier 2 (`api/gemini_client.py`) therefore cross-checks those two Flash-family models for now. This is a known, honest approximation of the original design, not a silent downgrade — it can be upgraded to a genuine cross-tier pair as soon as billing is enabled on the project; no architectural change would be needed beyond swapping `GEMINI_MODEL`/`GEMINI_SECONDARY_MODEL`.
- **`flagged_items` needs a second answer column for Tier 2 disagreements — not added yet.** When the two cross-check models disagree, `api/main.py` currently logs both candidate answers into the existing single `answer_text` column as one delimited text blob (`"[DISAGREEMENT]\nModel A: ...\nModel B: ..."`), rather than migrating the schema on a database file that already has real rows. This is a deliberate, temporary shortcut, not the intended shape: a proper `secondary_answer_text` column (plus a migration for existing rows) would let a future Tier 3 review tool parse and display both candidates cleanly instead of string-splitting a blob. Follow-up item, not blocking Tier 2 shipping.
- **Gemini's transient 503 "high demand" errors can occasionally cause a live escalation attempt to fail even when the pipeline is correctly configured.** Confirmed live against the deployed Render backend (2026-09-17): a direct request succeeded end-to-end, but two subsequent attempts through the actual deployed web app failed with `503 UNAVAILABLE` from `gemini-flash-latest` mid-outage-window. This is external to our system, not a bug — `ask_gemini()`'s retry logic (`api/gemini_client.py`, 2 retries with 1s/2s backoff) mitigates it but can't eliminate it if the outage outlasts the backoff window.
