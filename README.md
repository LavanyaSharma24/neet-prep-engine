...
An offline-first exam-prep PWA for NEET Biology & Chemistry, built around
one rule: **never guess**. Every answer is either pulled from a verified
item bank (works fully offline, installable on Android) or generated by
two independent AI models that must agree before it's shown — labeled
honestly either way.

**What it does:**
- Verified Q&A retrieval, works with zero internet after first load
- Optional on-device AI rephrasing ("Simplify this explanation") — runs
  entirely in-browser via `transformers.js`, no server round-trip
- Cross-checked cloud escalation for questions outside the item bank —
  two independent Gemini models must agree before an answer is served;
  disagreement is flagged for human review instead of guessing
- A few verified answers include diagrams (see below)

Built solo, using Claude Code alongside manual review/testing at every
step — see the commit history for the debugging trail, not just the
finished result.

v0 scope only (see `docs/PRD.md` §8 "Phased scope"): NEET Biology &
Chemistry, text-only, no voice. The terminal prototype
(`apps/prototype-cli/`) is pure Tier 0 keyword/fuzzy matching (Python
standard library only — no LLM, no network) with no diagrams and no
escalation; the deployed web app builds on the same core and adds
optional inline diagrams for a few verified items plus Tier 2 cloud
escalation for questions outside the item bank (see "Web app + API"
below). Either way, a match above the confidence threshold is answered
and labeled `VERIFIED`; anything else is refused and logged for later
...
- `item_bank/` — `schema.json` for one question item, `misconceptions/taxonomy.json`
  (empty skeleton, populated separately), `biology/neet_bio_items.jsonl` (9
  items) and `chemistry/*.jsonl` (30 items) — 39 items total.
